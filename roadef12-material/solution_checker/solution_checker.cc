...
RemainingCapacities::RemainingCapacities(const Capacities& initial_capacities,
  ...
  ROADEF_CHK_EQ(initial_capacities.size(), resources_.size());
...
void RemainingCapacities::Consume(const Requirements& requirements) {
  ...
  ROADEF_CHK_EQ(num_resources, ResourceIndex(requirements.size()));
...
int64 RemainingCapacities::GetLoadCost(
  ...
  int64 load_cost = 0;
  const ResourceIndex num_resources = GetNumberOfResources();
  ROADEF_CHK_EQ(num_resources, ResourceIndex(safety_remaining_capacities.size()));
  for (ResourceIndex resource_id(0);
       resource_id < num_resources;
       ++resource_id) {
...
Machine::Machine(MachineIndex id,
  ...
  ROADEF_CHK_EQ(num_resources, capacities_.size());
  ROADEF_CHK_EQ(num_resources, safety_remaining_capacities_.size());
  for (ResourceIndex resource_id(0);
       resource_id < num_resources;
       ++resource_id) {
...
void CheckAssigments(const ProcessAssignments& assignments,
  ...
  for (ProcessIndex process_id(0); process_id < num_processes; ++process_id) {
    ROADEF_CHK_LE(0, assignments.at(process_id));
    ROADEF_CHK_GT(num_machines, assignments.at(process_id));
  }
}
...
SolutionChecker::SolutionChecker(const Machines& machines,
  ...
  ROADEF_CHK_EQ(num_processes, num_processes_in_initial_assignments);
  ROADEF_CHK_EQ(num_processes, num_processes_in_new_assignments);
...
bool SolutionChecker::CheckRemainingCapacities() const {
  ...
  for (MachineIndex machine_id(0); machine_id < num_machines; ++machine_id) {
    ...
    if (machine.HasNegativeRemainingCapacity()) {
      ROADEF_LG << "Machine " << machine_id << " has a negative remaining capacity."
         << endl;
      return false;
    }
  }
...
vector<bool> is_machine_used(machines_.size(), false);
    ...
    for (LocalProcessIndex local_process_id(0);
      ...
      if (is_machine_used.at(machine_id)) {
        ROADEF_LG << "Service " << service_id << " has two processes running on "
...
vector<bool> is_location_used(machines_.size(), false);
    ...
    if (spread < spread_min) {
      ROADEF_LG << "Service " << service_id << " runs in " << spread
...
vector<bool> used_neighborhoods(machines_.size(),
  ...
  for (LocalProcessIndex local_process_id(0);
    ...
    if (!used_neighborhoods.at(neighborhood_id)) {
      ROADEF_LG << "Process " << process_id << " of service "
...
int DataParser::GetNextModelValue(int max_value) {
  ...
  if (next_value < 0) {
    ROADEF_LG << "Value at position " << raw_data_iterator_
  ...
  ROADEF_CHK(next_value >= 0 && next_value <= max_value);
...
void DataParser::GetModelVector(size_t size,
  ...
  ROADEF_CHK_NOTNULL(model_vector);
  model_vector->clear();
...
void DataParser::ParseModel() {
  ...
  ROADEF_CHK_EQ(raw_data_iterator_, raw_model_data_.size());
}
...
void DataParser::ParseAssignments(const vector<int>& assignments,
  ...
  ROADEF_CHK_NOTNULL(process_assignments);
