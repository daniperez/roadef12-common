...
#define ROADEF_CHK(x) assert(x)
#define ROADEF_CHK_EQ(x, y) ROADEF_CHK((x) == (y))
#define ROADEF_CHK_LE(x, y) ROADEF_CHK((x) <= (y))
#define ROADEF_CHK_GT(x, y) ROADEF_CHK((x) > (y))
#define ROADEF_CHK_NOTNULL(ptr) ROADEF_CHK(ptr);
#define ROADEF_LG cout
#define kint32max 0x7fffffff

template <class T>
void STLDeleteElements(vector<T*>* elements) {
  ROADEF_CHK_NOTNULL(elements);
